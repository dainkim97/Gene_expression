...
- Qualification (Salmon)
  ...
  2. Normalized based on TPM
     ...
     - (HPC) Filtering: awk 'NR==FNR {keep[$1]; next} FNR==1 || ($1 in keep)' keep_contigs_tpm1.txt supertranscript_nozero.matrix.tsv > supertranscript_counts.filtered_tpm1.matrix OR REF=/proj/omics/tepolt/Lobster_color_gene/backup/lobster_ref/rna.fna
KEEP=/user/dain.kim/tepolt/Dain/FOR_HYDRA/Mapping/Alignment/TPM_check/keep_contigs_tpm1.txt
OUTDIR=/user/dain.kim/tepolt/Dain/FOR_HYDRA/Mapping/Alignment/TPM_check/reindex_tpm1

mkdir -p "$OUTDIR"

awk -v KEEP="$KEEP" '
BEGIN {
  while ((getline < KEEP) > 0) keep[$1]=1
}
(/^>/) {
  id=$1
  sub(/^>/,"",id)
...
flag
' "$REF" > "$OUTDIR/filtered_tpm1_reference.fna" in log in node
